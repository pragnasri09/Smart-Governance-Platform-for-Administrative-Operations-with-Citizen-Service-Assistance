...
import in.gov.sgp.dto.Dtos.*;
import in.gov.sgp.model.*;
import in.gov.sgp.repository.*;
import in.gov.sgp.security.JwtService;
...
public class GovernanceService {
    ...
    public User current(Authentication a) {
        return users.findById(Long.valueOf(a.getName()))
                .orElseThrow(() ->
    ...
    public UserDto currentDto(Authentication a) {
        return userDto(current(a));
    }

    public UserDto userDto(User u) {
        var d = u.getDepartment();
    ...
    public DepartmentDto deptDto(Department d) {
    ...
    public ComplaintDto complaintDto(Complaint c) {
        var citizen = c.getCitizen();
        var dep = c.getDepartment();
        var staff = c.getAssignedStaff();
    ...
    public AuthResponse register(RegisterRequest x) {
        ...
        if (x.role() == Role.STAFF && x.departmentId() == null)
            throw new IllegalArgumentException(
                    "Staff registration requires a department."
            );

        if (users.findByEmailIgnoreCase(x.email().trim()).isPresent())
        ...
        u.setName(x.name().trim());
        u.setEmail(x.email().trim().toLowerCase());
        u.setPasswordHash(encoder.encode(x.password()));
        u.setRole(x.role());

        if (x.departmentId() != null)
            u.setDepartment(
                    departments.findById(x.departmentId())
                            .orElseThrow(() ->
    ...
    public AuthResponse login(LoginRequest x) {
        var u = users.findByEmailIgnoreCase(x.email().trim())
                .orElse(null);

        if (u == null ||
                !encoder.matches(x.password(), u.getPasswordHash()))
    ...
    public List<UserDto> listUsers(String search, Role role) {
        Stream<User> s =
                users.findAllByOrderByCreatedAtDesc().stream();
        ...
        if (role != null)
            s = s.filter(u -> u.getRole() == role);

        return s.map(this::userDto).toList();
    }
    ...
    public DepartmentDto createDepartment(DepartmentRequest x) {
        if (departments.findByNameIgnoreCase(x.name().trim()).isPresent())
            throw new DuplicateException(
        ...
        d.setDescription(x.description().trim());

        return deptDto(departments.save(d));
    }
    ...
    public void deleteDepartment(long id) {
        departments.findById(id).ifPresent(d -> {
            d.setActive(false);
            departments.save(d);
        });
    }

    private List<ComplaintDto> filter(
            List<Complaint> source,
            String search,
            ComplaintStatus status,
            Long dep
    ) {
        Stream<Complaint> s = source.stream();
        ...
        if (search != null && !search.isBlank()) {
            ...
            s = s.filter(c ->
                    ...
                    c.getCitizen().getName().toLowerCase().contains(q)
            );
        }

        if (status != null)
            s = s.filter(c -> c.getStatus() == status);

        if (dep != null)
            s = s.filter(
                    c -> c.getDepartment().getId().equals(dep)
            );

        return s.sorted(
                        Comparator.comparing(Complaint::getCreatedAt)
                                .reversed()
                )
                .map(this::complaintDto)
                .toList();
    }

    private Department routeDepartment(ComplaintRequest x) {
        String text = (
                x.category() + " " +
                x.title() + " " +
                x.description()
        ...
        String name =
                text.matches(".*(electric|power|street ?light).*")
                        ? "Electricity"
                        : text.matches(".*(water|leak|tap|supply).*")
                        ? "Water Supply"
                        : text.matches(".*(waste|garbage|sanit|clean).*")
                        ? "Sanitation"
                        : text.matches(".*(health|hospital|clinic|medical).*")
                        ? "Public Health"
                        : text.matches(".*(traffic|transport|signal|bus).*")
                        ? "Roads & Transport"
                        : text.matches(".*(road|pothole|drain|drainage|storm|flood).*")
                        ? "Public Works"
                        : null;
        ...
        return (
                name == null
                        ? departments.findById(x.departmentId())
                        : departments.findByNameIgnoreCase(name)
        )
                .filter(Department::isActive)
                .orElseThrow(() ->
                        new IllegalArgumentException(
                                "No active department is available for this complaint."
                        )
                );
    }

    private User autoAssign(Department d) {
        return users.findByRoleAndDepartmentOrderByCreatedAtAsc(
                        Role.STAFF,
                        d
                )
                .stream()
                .min(
                        Comparator.comparingLong(
                                complaints::countByAssignedStaff
                        )
                )
                .orElseThrow(() ->
                        new IllegalArgumentException(
                                "No service staff is available for this department."
                        )
                );
    }
    ...
    public ComplaintDto createComplaint(
        ...
        notify(
                ...
                "Complaint " +
                c.getReference() +
                " was submitted and assigned to " +
                c.getAssignedStaff().getName() +
                "."
        );
    ...
    public UserDto updateProfile(
            Authentication a,
            ProfileUpdateRequest x
    ) {
        var user = current(a);
        var email = x.email().trim().toLowerCase();
    ...
    public void deleteComplaint(long id) {
        var complaint = complaints.findById(id)
                .orElseThrow(() ->
    ...
    public List<ComplaintDto> myComplaints(
        ...
        return filter(
                complaints.findByCitizenOrderByCreatedAtDesc(current(a)),
                search,
                status,
                null
        );
    ...
    public List<ComplaintDto> assigned(
            Authentication a,
            String search,
            ComplaintStatus status
    ) {
        ...
        var all = complaints.findAllByOrderByCreatedAtDesc()
                .stream()
                .filter(c ->
                        Objects.equals(c.getAssignedStaff(), u) ||
                        (
                                c.getAssignedStaff() == null &&
                                c.getDepartment().equals(u.getDepartment()) &&
                                c.getStatus() == ComplaintStatus.PENDING
                        )
                )
                .toList();

        return filter(all, search, status, null);
    }

    public List<ComplaintDto> allComplaints(
            String search,
            ComplaintStatus status,
            Long dep
    ) {
    ...
    public ComplaintDto getComplaint(
        ...
        if (u.getRole() == Role.CITIZEN &&
                !c.getCitizen().getId().equals(u.getId()) ||
                u.getRole() == Role.STAFF &&
                (
                        c.getAssignedStaff() == null ||
                        !c.getAssignedStaff().getId().equals(u.getId())
                )
        )
            throw new NotFoundException("Complaint not found.");
    ...
    public ComplaintDto updateComplaint(
        ...
        if (!validTransition(c.getStatus(), x.status()))
            throw new IllegalArgumentException(
                    "Invalid status transition. Complaints must move from ASSIGNED to IN_PROGRESS, then to RESOLVED or REJECTED."
            );

        c.setStatus(x.status());

        if (x.remarks() != null)
            c.setRemarks(x.remarks());
    ...
    public List<NotificationDto> notifications(
        ...
        return notifications.findByUserOrderByCreatedAtDesc(current(a))
                .stream()
                .map(n ->
                        new NotificationDto(
                                n.getId(),
                                ...
                                n.getComplaint() == null
                                        ? null
                                        : n.getComplaint().getId(),
                                n.getComplaint() == null
                                        ? null
                                        : n.getComplaint().getReference(),
                                n.isRead(),
                                n.getCreatedAt()
                        )
                )
    ...
    public void contact(ContactRequest x) {
        var m = new ContactMessage();

        m.setName(x.name().trim());
        m.setEmail(x.email().trim());
        m.setSubject(x.subject().trim());
        m.setMessage(x.message().trim());
    ...
    public CitizenDashboard citizenDashboard(
        ...
        var u = current(a);
        var list = complaints.findByCitizenOrderByCreatedAtDesc(u);
    ...
    public StaffDashboard staffDashboard(
        ...
        var u = current(a);
        var list = complaints.findByAssignedStaffOrderByCreatedAtDesc(u);

        var overdue =
                complaints.countByAssignedStaffAndStatusNotInAndCreatedAtBefore(
                        u,
                        List.of(
    ...
    public AdminDashboard adminDashboard() {
        ...
        Map<String, Long> statuses = all.stream()
                .collect(
        ...
        var db = departments.findAll()
                .stream()
                .map(d ->
                        new DepartmentBreakdown(
                                d.getName(),
                                all.stream()
                                        .filter(c ->
                                                c.getDepartment().equals(d)
                                        )
                                        .count()
                        )
                )
        ...
        return new AdminDashboard(
                ...
                all.stream()
                        ...
                        .count(),
                departments.findByActiveTrueOrderByNameAsc().size(),
                statuses,
                db,
                all.stream()
                        .limit(5)
